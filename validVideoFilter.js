...
async function exportVideos() {
  ...
  try {
    ...
    for (let i = 0; i < videoDocs.length; i ++) {
      ...
      // 历史排重
      const saved = await db.validVideos.findOne({ videoUrl:videoDoc.videoUrl, currency: 'EGP' });
      if (saved) {
        console.log('视频已筛选过，无需重复筛选');
      } else {
        const productInfo = await db.products.findOne({ videoUrl: videoDoc.videoUrl });
        if (productInfo) {
          const inputPath = videoDoc.videoPath;
          const outputPath = path.resolve(__dirname, `download/${dateString}`, productInfo.originalId + '_' + productInfo.category3 + '_' + productInfo.price_EGP + '.mp4');
